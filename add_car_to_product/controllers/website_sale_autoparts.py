...
class WebsiteSaleAutoparts(http.Controller):
    ...
    @http.route("/shop/autoparts/models", type="http", auth="public", website=True)
    def autoparts_search(self,
                         brand_id=None,
                         model_id=None,
                         year=None,
                         volume_id=None,
        ...
        if model_id:
            vehicle_domain.append(("model_id", "=", int(model_id)))
        if year:
            year = int(year)
            vehicle_domain += [("model_year_from", "<=", year),
                               ("model_year_to", ">=", year)]
        if volume_id:
            vehicle_domain.append(("volume_id", "=", int(volume_id)))
